...
class C3gn(nn.Module):
    ...
    def __init__(self, c1, c2,  n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        # self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.gnconv=gnconv(2*c_,c2,n)
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])

    def forward(self, x):
        return self.gnconv(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
class CAGn(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2,  n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
    ...
    def forward(self, x):
        return self.gnconv(torch.cat((self.m(self.cv1(x)), self.ca(self.cv2(x))), dim=1))

class gnconv(nn.Module):
    def __init__(self, dim,out,order, gflayer=None, h=14, w=8, s=1.0):
        super().__init__()
        self.order = order+2
        ...
        self.proj_out = nn.Conv2d(dim, out, 1)
...
class C3HB(nn.Module):
    ...
    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))
class gnconv0(nn.Module):
    def __init__(self, dim,out,order=5, gflayer=None, h=14, w=8, s=1.0):
        super().__init__()
        self.order = order
        self.dims = [dim // 2 ** i for i in range(self.order)]
        self.dims.reverse()
...
class CoorAttention(nn.Module):
    ...
    def __init__(self, inp, oup, reduction=32):
        ...
        self.act = nn.ReLU()  # 这里自己可以实验什么激活函数最佳 论文里是hard-swish
